...
extern "C" BASICEXTERNALOBJECT_API long ExecAssemble(TaggedData* argv, long argc, TaggedData* retval)
{
	char proc_path[260] ={0};
	char proc_src [260] ={0};

	if(2 >=argc)
		return kESErrOK;

	strcpy(proc_path, argv [0].data.string);
	strcpy(proc_src , argv [1].data.string);



	//system("E:/zwork_img/pnginf.exe");
	system(proc_path);
	return kESErrOK;
}
